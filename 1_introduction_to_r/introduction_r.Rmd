...
`summary()` is able to produce different results because `summary()` is a ***generic function***. I won't discuss this in detail now, but suffice to say, how generic functions work depends on the type and attributes of the inputs you give them. 

One last note on factors. If you want to create a factor that has levels with a natural ordering (e.g. age group), you can create an "ordered" factor to preserve this ordering. Ordered factors make a number of calculations easier to carry out, which we'll make use of in future sessions.
...
As discussed, vectors can only hold one type of data at once: a vector cannot hold both character strings and numerics, for example. If you try to use two types of data in a single vector, one type will be *coerced* into another. See what happens when you run the following commands.
...
Now we can start learning a few more functions from the `tidyverse`. `select()` keeps certain columns from a `data.frame`. The code below keeps the columns `country`, `year`, and `lifeExp` from `gapminder` and discards the rest. (Note, because we haven't assigned the result, we haven't overwritten the original dataset.)
...
`select()` can also be used with the `everything()` helper function to rearrange the order of columns. Below we move `year` to the first column position (`everything()` returns all columns in a `data.frame`).
...
If you want to combine multiple statements with an `OR`, you'll need to combine them using the "`|`" operator.
...
The `mutate()` function is used to create new columns/variables in the `data.frame`. The new variable can be a function of other variables. Below we create two new columns for raw and log GDP. Note, when creating log GDP, we use the raw GDP variable created in the same function call. We are able to do this because, like `filter()`, `mutate()` works sequentially.
...
`group_by()` is used to split a `data.frame` into groups, so that calculations are carried out respecting the groups. `group_by()` is easier to explain by example. Below I group `gapminder` by `continent` to get median GDP and mean life expectancy figures for each continent separately. The results is a single `data.frame` with one row for each continent.
