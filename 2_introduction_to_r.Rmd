...
`select()` can also be used with the `everything()` "helper" function to rearrange the order of columns. Below we move `heart_attack` to the first column position (`everything()` returns all columns in a `data.frame`). (To see other helper functions, see [**this website**](https://tidyselect.r-lib.org/reference/index.html).)
